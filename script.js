...
function renderCommandCenter(data) {
    ...
    if (riskMap) {
        const rawRisks = data.risks_and_rebuttals || [];
        const score = data.final_score || 0;
        const seenRisks = new Set();
        const risks = rawRisks.filter(r => {
            const title = (r.adversarial_vector || r.attack_vector || r.risk || '').trim();
        ...
        let nodesHTML = `
            <div class="risk-node">
                <div class="risk-node-icon" style="background: var(--primary-100); color: var(--primary-600);"><i class="fas fa-landmark"></i></div>
                <div class="risk-node-info"><h5>Case Origin Data</h5><p>Verified Pillars: ${data.compliance_pct || 0}%</p></div>
            </div>
        `;
        risks.forEach(r => {
            nodesHTML += `
            <div class="risk-node">
                ...
                <div class="risk-node-info">
                    <h5>${escapeHTML(r.adversarial_vector || r.risk || 'Risk Vector')}</h5>
                    <p>Survivability: ${escapeHTML(r.survival_probability || '65%')}</p>
                </div>
            </div>`;
        });
        nodesHTML += `
            <div class="risk-node">
                ...
                <div class="risk-node-info"><h5>Final Verdict Posture</h5><p>Conviction Index: ${score}%</p></div>
            </div>
        `;
    ...
    if (causalityPanel) {
        const causality = cleanList(data.causality_map || []);
        causalityPanel.innerHTML = causality.length ? causality.map(c => `
            <div class="deduction-chip">
                <span class="penalty-reason">${escapeHTML(c.fact)}</span>
                <span class="penalty-value" style="color: ${c.impact >= 0 ? 'var(--success-600)' : 'var(--error-600)'}">
                    ${c.impact >= 0 ? '+' : ''}${c.impact}
                </span>
            </div>
        `).join('') : '<p class="rl-empty">No causal factors detected.</p>';
    }
    ...
    if (roadmapEl) {
        ...
        roadmapEl.innerHTML = stages.map((s, i) => `
            <div class="roadmap-step ${parseFloat(s.prob) < 50 ? 'urgent' : ''}">
                <h4><i class="fas fa-layer-group"></i> ${escapeHTML(s.name)} <span style="margin-left:auto; color:var(--primary-600)">${s.prob}</span></h4>
    ...
    if (econEl) {
        ...
        econEl.innerHTML = `
            <div class="economics-item" style="display: flex; justify-content: space-between; padding: 0.75rem 0; border-bottom: 1px solid var(--gray-100);">
                ...
                <span style="font-weight: 700; color: var(--gray-900);">${escapeHTML(econ.immediate_settlement)}</span>
            </div>
            <div class="economics-item" style="display: flex; justify-content: space-between; padding: 0.75rem 0; border-bottom: 1px solid var(--gray-100);">
                <span style="font-size: 0.85rem; color: var(--gray-600);">Trial Target (36 mo)</span>
                <span style="font-weight: 700; color: var(--success-600);">${escapeHTML(econ.trial_target_3yr)}</span>
            </div>
            <div class="economics-item" style="display: flex; justify-content: space-between; padding: 0.75rem 0;">
                ...
                <span style="font-weight: 800; color: ${econ.settlement_posture === 'AGGRESSIVE' ? 'var(--primary-600)' : 'var(--warning-600)'}">${escapeHTML(econ.settlement_posture)}</span>
            </div>
        `;
    }
    ...
    if (checkpointsEl) {
        const checkpoints = cleanList(data.checkpoints || []);
        checkpointsEl.innerHTML = checkpoints.length ? checkpoints.map(cp => `
            <div class="checkpoint-item" style="display: flex; flex-direction: column; gap: 0.25rem; margin-bottom: 0.75rem; font-size: 0.85rem; padding: 0.75rem; background: #fff; border-radius: 8px; border-left: 4px solid ${cp.priority === 'CRITICAL' ? 'var(--error-500)' : 'var(--primary-500)'}; box-shadow: var(--shadow-sm);">
                <div style="display: flex; justify-content: space-between; align-items: center;">
                    <strong style="color: var(--gray-900);">${escapeHTML(cp.task)}</strong>
                ...
                <div style="display: flex; justify-content: space-between; align-items: center; margin-top: 0.25rem;">
                    <span style="font-size: 11px; color: var(--gray-500);"><i class="fas fa-info-circle"></i> Status: ${escapeHTML(cp.status)}</span>
                </div>
            </div>
        `).join('') : '<p class="rl-empty">No mandatory checkpoints identified.</p>';
    }

    const rebuttalEl = document.getElementById('defenceRebuttalTree');
    if (rebuttalEl) {
        const risks = cleanList(data.risks_and_rebuttals || []);
        rebuttalEl.innerHTML = risks.length ? risks.map(r => `
            <div class="rebuttal-node" style="background: white; padding: 1.25rem; border-radius: var(--radius-lg); border: 1px solid var(--gray-200); margin-bottom: 1rem; box-shadow: var(--shadow-sm);">
                <div style="font-weight: 800; font-size: 0.9rem; margin-bottom: 0.5rem; color: var(--error-700);">
                    <i class="fas fa-skull-crossbones"></i> DEFENCE ATTACK: ${escapeHTML(r.attack_vector || r.adversarial_vector || 'Generic Attack')}
                </div>
                <div class="rebuttal-step" style="font-size: 0.85rem; margin-bottom: 0.4rem; padding-left: 0.75rem; border-left: 2px solid var(--gray-200);"><strong>Attack:</strong> ${escapeHTML(r.tactical_chain ? r.tactical_chain[0] : (r.strategic_chain ? r.strategic_chain[0] : 'Generic attack'))}</div>
                <div class="rebuttal-step" style="font-size: 0.85rem; margin-bottom: 0.4rem; padding-left: 0.75rem; border-left: 2px solid var(--success-400);"><strong>Rebuttal:</strong> ${escapeHTML(r.rebuttal_tree ? r.rebuttal_tree.complainant_counter : 'Counter with S.139')}</div>
                <div class="magistrate-view" style="font-size: 0.8rem; background: var(--gray-50); padding: 0.5rem; border-radius: 4px; margin-top: 0.5rem; color: var(--gray-600);">
                    <i class="fas fa-gavel"></i> Magistrate View: ${escapeHTML(r.rebuttal_tree ? r.rebuttal_tree.magistrate_view : 'Probabilistic assessment.')}
                </div>
            </div>
        `).join('') : '<p class="rl-empty">No defence trees detected.</p>';
    }
    ...
    if (pressEl) {
        ...
        pressEl.innerHTML = `
            ...
            <div style="margin-top: 0.75rem; padding: 0.5rem; background: ${color}11; border-radius: 4px; text-align: center; font-size: 0.75rem; font-weight: 800; color: ${color};">
                BREAKDOWN RISK: ${escapeHTML(wp.breakdown_risk || 'LOW')}
            </div>
        `;
    }
    ...
    if (redEl) {
        ...
        redEl.innerHTML = attacks.length ? attacks.map(a => `
            <div style="background: rgba(255,255,255,0.05); padding: 1.25rem; border-radius: 12px; border: 1px solid rgba(255,255,255,0.1); border-left: 4px solid #ef4444;">
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 0.5rem;">
                    <span style="font-weight: 900; color: #fca5a5; font-size: 0.75rem; text-transform: uppercase;">TARGET: ${escapeHTML(a.target)}</span>
                    <span style="font-size: 0.65rem; font-weight: 900; padding: 0.15rem 0.5rem; border-radius: 4px; background: #ef4444; color: white;">${escapeHTML(a.lethality)}</span>
                </div>
                <div style="font-weight: 700; font-size: 1rem; margin-bottom: 0.5rem;">${escapeHTML(a.attack)}</div>
                <div style="background: rgba(0,0,0,0.3); padding: 0.75rem; border-radius: 6px; font-family: monospace; font-size: 0.85rem; color: #fecaca;">
                    ...
                    ${escapeHTML(a.hostile_argument)}
                </div>
            </div>
        `).join('') : '<div style="padding: 2rem; text-align: center; opacity: 0.6;">No fatal vulnerabilities detected for exploitation.</div>';
    }
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Initial tab state
    window.activeResultTab = 'overview';

    // Visibility Listener for Sync Optimization
    document.addEventListener('visibilitychange', () => {
        if (document.visibilityState === 'visible' && typeof currentCaseroomId !== 'undefined' && currentCaseroomId) {
            if (typeof refreshCaseroom === 'function') refreshCaseroom();
        }
    });
...
async function processPayment() {
    ...
    setTimeout(() => {
        ...
        btn.disabled = false;
        closePaymentModal();
        showToast('Payment successful! Your account has been upgraded.', 'success', 'Payment Complete');
        // In a real app, update user role in database here
...
function showRefund() {
    hideAllScreens();
    document.getElementById('refundScreen').classList.remove('hidden');
    window.scrollTo(0, 0);
}
...
async function handleRegister(form) {
    ...
    try {
        ...
        const termsChecked = form.querySelector('#registerTerms').checked;
        if (!termsChecked) {
...
function switchResultTab(tabName) {
    // 🔥 STEP 8: Tab-Aware Intelligence Sync
    window.activeResultTab = tabName;
    ...
    // If switching TO caseroom, ensure sync is active
    if (tabName === 'caseroom' && currentCaseroomId) {
        if (typeof startCaseroomSync === 'function') {
            startCaseroomSync();
        }
    }
}
...
function renderFullReport(data) {
    ...
    if (data.caseroom_id) {
        ...
        // Optimization: Guard against redundant sync restarts
        if (typeof startCaseroomSync === 'function') {
            startCaseroomSync();
        }
    }
    ...
    // 🔥 REGULATORY FIX (2026 Audit): Ensure 'Defence Mode' is default for weak cases
    if (data.score < 50 && data.draft) {
        console.log("⚠️ [REGULATORY] Weak case detected (Score < 50). Defaulting to Defence Mode for drafting.");
        setTimeout(() => setDraftTone('defence'), 100);
    }

    // 🏛️ JURISDICTION MAPPING (S.142 Post-2015 Amendment)
    renderJurisdictionPanel(data);
}
/**
...
function setDraftTone(tone) {
    ...
    const toneMaps = {
        ...
        defence: [
            ["demand that you pay", "contesting the alleged liability"],
            ["initiate criminal proceedings", "rebutting the false claims in court"],
            ["failure to make payment", "absence of any legally enforceable debt"],
            ["Negotiable Instruments Act", "limitations and procedural defects of the Negotiable Instruments Act"],
            ["Sir/Madam", "To the Complainant/Counsel"],
...
/**
 * 🔥 NEW: Automated Utility to regenerate specific drafts (BSA Certificate, etc.)
 */
async function regenerateDraft(targetType) {
    if (!window.formData) {
        showToast("No case data found to generate draft.", "error");
        return;
    }

    const draftPreviewEl = document.getElementById("draftPreviewContent");
    const originalText = draftPreviewEl.value;
    draftPreviewEl.value = "🔄 Regenerating specialized draft...\n\nPlease wait while JudiQ AI prepares your " + targetType.replace('_', ' ') + "...";

    showToast(`Generating ${targetType.replace('_', ' ')}...`, "info");

    try {
        const payload = { ...window.formData, force_draft_type: targetType };
        ...
        if (result.success && result.draft) {
            draftPreviewEl.value = result.draft;
            // Also update the global caseData so tone switching works on the new draft
            if (window.caseData) window.caseData.draft = result.draft;

            showToast(`${targetType.replace('_', ' ')} generated successfully.`, "success");
        ...
        draftPreviewEl.value = originalText;
    }
}
...
// ═══════════════════════════════════════════════════════════════════════════
// 🏛️ JURISDICTION MAPPING PANEL (S.142 NI Act — Post-2015 Amendment)
// ═══════════════════════════════════════════════════════════════════════════

function renderJurisdictionPanel(data) {
    const el = document.getElementById('jurisdictionPanel');
    if (!el) return;

    const j = data.jurisdiction;
    if (!j || j.status === 'INSUFFICIENT_DATA') {
        el.innerHTML = `
        <div style="background:var(--gray-50);border:1px solid var(--gray-200);border-radius:0.75rem;padding:1.25rem 1.5rem;margin-top:1rem;">
            <div style="display:flex;align-items:center;gap:0.6rem;margin-bottom:0.5rem;">
                <i class="fas fa-map-marker-alt" style="color:var(--gray-400);"></i>
                <strong style="font-size:0.9rem;color:var(--gray-600);">Jurisdiction</strong>
                <span style="font-size:0.75rem;background:var(--gray-200);color:var(--gray-600);padding:0.15rem 0.6rem;border-radius:999px;">DATA NEEDED</span>
            </div>
            <p style="font-size:0.82rem;color:var(--gray-500);margin:0;">Provide the payee bank branch city to get the correct court under S.142(2) NI Act (2015 Amendment).</p>
        </div>`;
        el.classList.remove('hidden');
        return;
    }

    const confidenceColors = { HIGH: '#059669', MEDIUM: '#d97706', LOW: '#dc2626' };
    const confidenceColor = confidenceColors[j.confidence] || '#64748b';

    const warningsHtml = (j.warnings || []).map(w => `
        <div style="display:flex;align-items:flex-start;gap:0.5rem;margin-top:0.4rem;font-size:0.8rem;color:#92400e;background:#fffbeb;border-radius:0.4rem;padding:0.5rem 0.75rem;">
            <i class="fas fa-exclamation-triangle" style="color:#d97706;margin-top:0.1rem;flex-shrink:0;"></i>
            <span>${w}</span>
        </div>`).join('');

    const checklistHtml = (j.filing_checklist || []).map(item => `
        <div style="font-size:0.8rem;color:var(--gray-700);padding:0.25rem 0;display:flex;align-items:flex-start;gap:0.5rem;">
            <i class="fas fa-check-circle" style="color:#059669;margin-top:0.15rem;flex-shrink:0;font-size:0.75rem;"></i>
            <span>${item.replace(/^✅\s*/, '')}</span>
        </div>`).join('');

    const alternatesHtml = (j.alternate_courts || []).map(ac => `
        <div style="font-size:0.78rem;padding:0.35rem 0.6rem;border-radius:0.4rem;background:var(--gray-50);border-left:3px solid ${ac.applicability.includes('OVERRULED') ? '#dc2626' : '#94a3b8'};margin-top:0.4rem;">
            <strong>${ac.court}</strong><br>
            <span style="color:${ac.applicability.includes('OVERRULED') ? '#dc2626' : 'var(--gray-500)'};">${ac.applicability}</span> — ${ac.basis}
        </div>`).join('');

    el.innerHTML = `
    <div style="background:linear-gradient(135deg,#f0fdf4 0%,#ecfdf5 100%);border:1.5px solid #a7f3d0;border-radius:0.875rem;padding:1.5rem;margin-top:1rem;">
        <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:0.5rem;margin-bottom:1rem;">
            <div style="display:flex;align-items:center;gap:0.7rem;">
                <div style="width:36px;height:36px;border-radius:50%;background:#059669;display:flex;align-items:center;justify-content:center;">
                    <i class="fas fa-balance-scale" style="color:#fff;font-size:0.9rem;"></i>
                </div>
                <div>
                    <div style="font-size:0.75rem;font-weight:600;color:#059669;letter-spacing:0.05em;text-transform:uppercase;">S.142 Jurisdiction Mapping</div>
                    <div style="font-size:1rem;font-weight:700;color:var(--gray-900);">${j.recommended_court}</div>
                </div>
            </div>
            <span style="font-size:0.72rem;font-weight:700;background:${confidenceColor}18;color:${confidenceColor};padding:0.25rem 0.75rem;border-radius:999px;border:1px solid ${confidenceColor}44;">
                ${j.confidence} CONFIDENCE
            </span>
        </div>

        <div style="font-size:0.82rem;color:var(--gray-700);line-height:1.6;background:#fff;border-radius:0.5rem;padding:0.75rem 1rem;border:1px solid #d1fae5;margin-bottom:0.75rem;">
            <i class="fas fa-info-circle" style="color:#059669;margin-right:0.4rem;"></i>
            <strong>Legal Basis:</strong> ${j.legal_basis}
        </div>

        <div style="font-size:0.78rem;color:var(--gray-500);margin-bottom:0.75rem;">
            <i class="fas fa-gavel" style="margin-right:0.4rem;"></i>
            <em>${j.key_ruling}</em>
        </div>

        ${warningsHtml}

        ${j.filing_checklist && j.filing_checklist.length ? `
        <div style="margin-top:0.75rem;border-top:1px solid #d1fae5;padding-top:0.75rem;">
            <div style="font-size:0.78rem;font-weight:600;color:var(--gray-600);margin-bottom:0.4rem;text-transform:uppercase;letter-spacing:0.05em;">Filing Checklist</div>
            ${checklistHtml}
        </div>` : ''}

        ${j.alternate_courts && j.alternate_courts.length ? `
        <div style="margin-top:0.75rem;border-top:1px solid #d1fae5;padding-top:0.75rem;">
            <div style="font-size:0.78rem;font-weight:600;color:var(--gray-600);margin-bottom:0.4rem;text-transform:uppercase;letter-spacing:0.05em;">Alternative / Overruled Courts</div>
            ${alternatesHtml}
        </div>` : ''}

        <div style="margin-top:1rem;display:flex;gap:0.5rem;flex-wrap:wrap;">
            <button onclick="selectDraftType('bsa_63_4_certificate'); openDraftGeneratorScreen('results');" 
                    style="font-size:0.78rem;padding:0.45rem 1rem;background:#7c3aed;color:#fff;border:none;border-radius:0.5rem;cursor:pointer;display:flex;align-items:center;gap:0.4rem;">
                <i class="fas fa-shield-halved"></i> Generate BSA 63(4) Certificate
            </button>
            <button onclick="switchResultTab('draft')" 
                    style="font-size:0.78rem;padding:0.45rem 1rem;background:#059669;color:#fff;border:none;border-radius:0.5rem;cursor:pointer;display:flex;align-items:center;gap:0.4rem;">
                <i class="fas fa-file-contract"></i> Draft Complaint for ${j.primary_city || 'this Court'}
            </button>
        </div>
    </div>`;
    el.classList.remove('hidden');
...
function displayTimeline(rawTimeline) {
    const container = document.getElementById('timelineList');
    if (!container) return; // Skip if element doesn't exist

    const timeline = cleanList(rawTimeline);
            ...
            <div class="timeline-item" style="position: relative; padding-left: 2rem; padding-bottom: 1.5rem; border-left: 2px solid var(--gray-300);">
                ...
                <div class="timeline-content" style="background: white; padding: 1rem; border-radius: 0.5rem; box-shadow: var(--shadow-sm);">
                    <div style="display: flex; justify-content: space-between; align-items: start; margin-bottom: 0.5rem;">
                        <strong style="color: var(--gray-900); font-size: 0.95rem;">${escapeHTML(eventText)}</strong>
                        ${date ? `<span style="font-size: 0.75rem; color: var(--gray-600); white-space: nowrap; margin-left: 1rem;"><i class="fas fa-calendar"></i> ${escapeHTML(date)}</span>` : ''}
                    </div>
                    ${description ? `<p style="font-size: 0.875rem; color: var(--gray-600); margin-top: 0.5rem;">${escapeHTML(description)}</p>` : ''}
...
function displayStrategy(rawStrategy) {
    const container = document.getElementById('strategyContent');
    if (!container) return; // Skip if element doesn't exist

    const strategy = Array.isArray(rawStrategy) ? cleanList(rawStrategy) : rawStrategy;
...
function displayDefences(rawDefences) {
    const container = document.getElementById('defencesList');
    const defences = cleanList(rawDefences);
...
function displaySemanticAnalysis(semantic) {
    ...
    const rawConcepts = semantic.concepts_detected || [];
    const concepts = cleanList(rawConcepts);
...
function renderExecutiveSummary() {
    ...
    return `
        <table class="report-table">
            ...
            <tr><th>Analysis Confidence</th><td>${(data.score_metadata?.confidence_interval ? 'HIGH' : 'STABLE')}</td></tr>
...
// ── DATA HYGIENE UTILITY (Kill Repetition & Blanks) ──────────────────────────
...
function renderList(id, rawItems, fallback = "No data available") {
    const el = document.getElementById(id);
    if (!el) return; // Element doesn't exist

    const cleanItems = cleanList(rawItems);
    ...
    el.innerHTML = cleanItems.map((item, index) => {
        let displayText;
...
function hideAllScreens() {
    const screens = [
        ...
        'termsScreen', 'privacyScreen', 'refundScreen'
    ];
...
// ─── Type 13: BSA 63(4) Certificate (Bharatiya Sakshya Adhiniyam, 2023) ──────
// Mandatory for digital evidence admissibility. Replaces S.65B Indian Evidence Act.
const BSA_DRAFT_TYPE = {
    id: 'bsa_63_4_certificate', number: 13, title: 'BSA Section 63(4) Certificate', subtitle: 'Digital Evidence',
    description: 'Mandatory under Bharatiya Sakshya Adhiniyam for admissibility of WhatsApp, Email & SMS evidence. Without this, all digital records are inadmissible.',
    icon: 'fa-shield-halved', color: '#7c3aed',
    fields: [
        { name: 'complainant_name', label: 'Deponent / Complainant Full Name', type: 'text', required: true, placeholder: 'e.g., Ramesh Kumar Sharma' },
        { name: 'complainant_address', label: 'Deponent Full Address', type: 'textarea', required: true, placeholder: 'Full permanent residential address...' },
        { name: 'accused_name', label: 'Accused Full Name', type: 'text', required: true, placeholder: 'e.g., Suresh Mohan Gupta' },
        { name: 'court_name', label: 'Court Name & Location', type: 'text', required: true, placeholder: 'e.g., Judicial Magistrate First Class, Pune' },
        { name: 'case_number', label: 'Case / Complaint No. (if filed)', type: 'text', required: false, placeholder: 'Leave blank if filing before the complaint' },
        { name: 'device_type', label: 'Electronic Device Used', type: 'select', required: true, options: ['Smartphone (Android)', 'Smartphone (iPhone/iOS)', 'Personal Computer (Windows)', 'Personal Computer (Mac)', 'Tablet', 'WhatsApp Business Account', 'Email Server / Business Account'] },
        { name: 'device_owner', label: 'Device Owner / Custodian', type: 'select', required: true, options: ['Complainant (Self)', "Complainant's Authorized Employee", 'Third-Party Custodian'] },
        { name: 'evidence_type', label: 'Type of Electronic Evidence', type: 'select', required: true, options: ['WhatsApp Messages / Chat History', 'Email Correspondence', 'SMS / Text Messages', 'Voice Call Records (CDR)', 'WhatsApp + Email (Combined)', 'Bank Transaction SMS / OTP Records'] },
        { name: 'period_from', label: 'Evidence Period — From Date', type: 'date', required: true },
        { name: 'period_to', label: 'Evidence Period — To Date', type: 'date', required: true },
        { name: 'annexure_number', label: 'Annexure Number for Digital Records', type: 'text', required: true, placeholder: 'e.g., Annexure-F or Exhibit-3' },
        { name: 'deposition_place', label: 'Place of Verification / Deposition', type: 'text', required: true, placeholder: 'e.g., Pune, Maharashtra' },
        { name: 'certificate_date', label: 'Date of This Certificate', type: 'date', required: true }
    ],
    generate: function(d) {
        const fmt = (dateStr) => {
            if (!dateStr) return '_______________';
            try { return new Date(dateStr).toLocaleDateString('en-IN', { day: '2-digit', month: 'long', year: 'numeric' }); } catch(e) { return dateStr; }
        };
        const bar = '='.repeat(72);
        const dash = '\u2500'.repeat(72);
        const caseRef = d.case_number ? `Case No. ${d.case_number}` : 'Complaint (Pre-Filing Stage)';
        return `CERTIFICATE UNDER SECTION 63(4) OF THE BHARATIYA SAKSHYA ADHINIYAM, 2023
(Replaces Section 65B, Indian Evidence Act — Mandatory for Electronic Records)

${bar}
IN THE COURT OF THE LEARNED ${d.court_name.toUpperCase()}
...
AFFIDAVIT-CUM-CERTIFICATE UNDER SECTION 63(4) OF THE
BHARATIYA SAKSHYA ADHINIYAM (BSA), 2023
FOR ADMISSIBILITY OF ELECTRONIC RECORDS

I, ${d.complainant_name}, adult Indian citizen, residing at
${d.complainant_address},
do hereby solemnly affirm, declare and certify on oath as follows:

1. IDENTITY & COMPETENCE:
   I am the Complainant / Deponent in the above matter. I am personally
   acquainted with all facts of the case and am fully competent to swear
   to this affidavit.

2. NATURE OF ELECTRONIC RECORDS:
   For this case, I rely upon electronic records in the form of
   ${d.evidence_type}, exchanged between myself and the Accused
   (${d.accused_name}), as stored on my ${d.device_type}.

3. DEVICE PARTICULARS (S.63(4)(a)):
   The electronic records were generated and stored on a ${d.device_type},
   which was at all material times owned/operated by the ${d.device_owner}.
   The device was used regularly in the ordinary course of my activities.

4. REGULAR USE & NORMAL OPERATION (S.63(4)(b)):
   During the period from ${fmt(d.period_from)} to ${fmt(d.period_to)}
   ("the Material Period"), information was regularly fed into the said
   device in the ordinary course of normal activities. The device was
   operating properly throughout, and any temporary non-operation during
   the Material Period did not affect the accuracy of the records.

5. AUTOMATED DATA ENTRY (S.63(4)(c)):
   The electronic records were generated automatically by the device in
   the ordinary course of its operation and were not manually fabricated.

6. REPRODUCTION ACCURACY (S.63(4)(d)):
   The screenshots/printouts produced herewith as ${d.annexure_number}
   are true and faithful reproductions of the originals as they appear
   on the said device, prepared under my personal supervision.

7. NO TAMPERING OR MODIFICATION:
   The electronic records have not been tampered with, altered, or
   manipulated in any manner between the date of original generation
   and the date of production before this Hon'ble Court.

8. STATUTORY COMPLIANCE:
   This Certificate is issued in strict compliance with Section 63(4)
   of the Bharatiya Sakshya Adhiniyam, 2023, to certify the electronic
   records annexed herewith as ${d.annexure_number} as admissible
   evidence before this Hon'ble Court.

9. TRUTH OF CONTENTS:
   The contents of this Certificate are true and correct to the best of
   my knowledge and belief. Nothing material has been concealed herein.

DEPONENT: ${d.complainant_name}

VERIFICATION:
Verified at ${d.deposition_place} on this ${fmt(d.certificate_date)} that
the contents of the above Affidavit-cum-Certificate are true and correct
...
Notarized / Sworn Before:
____________________________
Notary Public / Oath Commissioner

${dash}
MANDATORY ADVOCATE CHECKLIST — DO NOT FILE WITHOUT THIS:
[ ] Certificate is sworn before a Notary / Oath Commissioner (NOT self-attested)
[ ] Annexure ${d.annexure_number} contains certified screenshots/exports from the device
[ ] Device IMEI / Phone Number is documented in your brief notes
[ ] Screenshots are time-stamped and unedited
[ ] Certificate filed ALONG WITH the complaint, not separately later
LEGAL BASIS: Anvar P.V. vs. P.K. Basheer (2014) 10 SCC 473 — principle upheld
under BSA 2023. Shafhi Mohammad vs. State of HP (2018) SC — clarified.
${dash}`;
    }
};
DRAFT_TYPES.push(BSA_DRAFT_TYPE);
...
// Global UI State
let currentExperienceMode = localStorage.getItem('judiq_experience_mode') || 'executive';
...
function applyExperienceMode() {
    const isExecutive = currentExperienceMode === 'executive';
    ...
    forensicSections.forEach(id => {
        const el = document.getElementById(id);
        if (el) {
            const container = el.closest('.result-card') || el;
            if (isExecutive) {
...
function renderQuestionBank(data) {
    ...
    const rawQuestions = data.question_bank || [];
    const questions = cleanList(rawQuestions);

    if (questions.length === 0) {
        el.innerHTML = '<div class="evidence-empty">No adversarial questions generated for this posture.</div>';
        return;
    }
    ...
    win.document.write(`
        ...
        <body>
            ...
            <div class="grid">
                ...
                <div class="item"><div class="label">Case Pattern</div><div class="val">${data.case_similarity?.pattern_matched}</div></div>
            </div>
...
function renderAIReasoningLayer(data) {
    if (!data) return;
    console.log('[INTELLIGENCE] Rendering AI Reasoning Layer...');
    ...
    if (execSummaryEl) {
        // High-impact Executive Summary (v3)
        ...
        execSummaryEl.innerHTML = `
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem; margin-top: 0.5rem;">
                <div style="padding: 0.75rem; background: rgba(255,255,255,0.05); border-radius: 8px;">
                    <div style="font-size: 0.7rem; font-weight: 800; color: rgba(255,255,255,0.5); text-transform: uppercase;">Core Risk Profile</div>
                    <div style="font-size: 1.1rem; font-weight: 700; color: white;">${tldr.core_risk || 'Standard'}</div>
                </div>
                <div style="padding: 0.75rem; background: rgba(255,255,255,0.05); border-radius: 8px;">
                    <div style="font-size: 0.7rem; font-weight: 800; color: rgba(255,255,255,0.5); text-transform: uppercase;">Top Strategy Threat</div>
                    <div style="font-size: 1.1rem; font-weight: 700; color: white;">${tldr.top_threat || 'None'}</div>
                </div>
                <div style="padding: 0.75rem; background: rgba(255,255,255,0.05); border-radius: 8px;">
                    <div style="font-size: 0.7rem; font-weight: 800; color: rgba(255,255,255,0.5); text-transform: uppercase;">Best Remediation</div>
                    <div style="font-size: 1.1rem; font-weight: 700; color: white;">${tldr.best_move || 'Standard'}</div>
                </div>
                <div style="padding: 0.75rem; background: rgba(255,255,255,0.05); border-radius: 8px;">
                    <div style="font-size: 0.7rem; font-weight: 800; color: rgba(255,255,255,0.5); text-transform: uppercase;">Case Confidence</div>
                    <div style="font-size: 1.1rem; font-weight: 700; color: white;">${tldr.confidence || '0%'}</div>
                </div>
            </div>
            <div style="margin-top: 1rem; font-style: italic; color: rgba(255,255,255,0.9); font-size: 0.95rem; border-left: 3px solid var(--primary-400); padding-left: 1rem;">
                "${tldr.one_liner || 'Assessment complete.'}"
            </div>
        `;
    }
    ...
    console.log('[SUCCESS] AI Reasoning Layer rendered successfully.');
    ...
    `;
        ...
        risks.forEach(r => {
            const riskTitle = (r.adversarial_vector || r.attack_vector || r.risk || '').trim();
            nodesHTML += `
            <div class="risk-node">
                <div class="risk-node-icon" style="background: var(--error-100); color: var(--error-600);"><i class="fas fa-bolt"></i></div>
                <div class="risk-node-info">
                    <h5>${escapeHTML(riskTitle)}</h5>
                    <p>Lethality: ${r.lethality || r.severity || 'MED'}</p>
                </div>
            </div>
            `;
        });
...
function startCaseroomSync() {
    ...
    // Guard: Don't restart if already syncing the same room
    if (caseroomInterval && window.lastActiveSyncRoomId === currentCaseroomId) {
        console.log(`[CASEROOM] Sync already active for ${currentCaseroomId}`);
        return;
    }

    if (caseroomInterval) clearInterval(caseroomInterval);
    window.lastActiveSyncRoomId = currentCaseroomId;

    console.log(`[CASEROOM] Initializing Sync for ${currentCaseroomId}...`);
    refreshCaseroom();

    // Dynamic Polling: 5s if active tab, 15s if background
    caseroomInterval = setInterval(() => {
        const interval = window.activeResultTab === 'caseroom' ? 5000 : 15000;
        // If we want truly dynamic, we'd need to clear and restart, 
        // but 5s is safe for now if we add a "background" check inside refresh.
        refreshCaseroom();
    }, 5000);
}
...
async function refreshCaseroom() {
    ...
    // Optimization: Skip if page is hidden to save API calls
    ...
    try {
        ...
        const result = await response.json();
        if (result.success) {
            // Hash check to prevent redundant DOM updates
            const currentHash = JSON.stringify(result.data.messages || []) + (result.data.documents || []).length;
            if (currentHash === lastSyncDataHash) {
                return; // No changes, skip render
            }
            lastSyncDataHash = currentHash;

            renderCaseroom(result.data);

            // 🔥 INTELLIGENCE SYNC: Check if backend analysis has updated
            if (result.data.latest_analysis && result.data.latest_analysis.request_id !== (window.caseData ? window.caseData.request_id : '')) {
                console.log("🔥 [SYNC] New intelligence detected in Caseroom. Updating dashboard...");
                window.caseData = result.data.latest_analysis;
                renderFullReport(window.caseData);
            }
        }
    } catch (error) {
...
function renderCaseroom(data) {
    ...
    if (data.messages && data.messages.length > 0) {
        chatBox.innerHTML = data.messages.map(m => {
            ...
            <div class="chat-bubble ${type}">
                <div class="chat-meta">${escapeHTML(m.user_id)} • ${new Date(m.timestamp).toLocaleTimeString()}</div>
    ...
    // Render Evidence Board (NEW)
    const evidenceBoard = document.getElementById('evidenceBoard');
    if (evidenceBoard) {
        if (data.documents && data.documents.length > 0) {
            evidenceBoard.innerHTML = data.documents.map(d => `
                <div class="evidence-card" onclick="viewDocument('${d.file_name}')">
                    <div class="evidence-icon"><i class="fas ${d.doc_type === 'MEMO' ? 'fa-university' : (d.doc_type === 'CHEQUE' ? 'fa-money-check-alt' : 'fa-file-contract')}"></i></div>
                    <div class="evidence-info">
                        <div class="evidence-name">${escapeHTML(d.file_name)}</div>
                        <div class="evidence-status verified">${d.validation_status}</div>
                    </div>
                </div>
            `).join('');
        } else {
            evidenceBoard.innerHTML = '<div class="evidence-empty">No documents uploaded yet.</div>';
        }
    }
}
...
function triggerCaseroomUpload() {
    const input = document.getElementById('caseroomFileInput');
    if (input) input.click();
}

async function handleCaseroomUpload(event) {
    const file = event.target.files[0];
    if (!file || !currentCaseroomId) return;

    const formData = new FormData();
    formData.append('file', file);
    formData.append('user_id', currentUser?.uid || 'LEAD_ADVOCATE');
    formData.append('doc_type', 'EVIDENCE'); // Default
    formData.append('claimed_reason', 'None');

    try {
        showAnalysisLoading('Uploading & Analyzing Evidence...');
        const response = await fetch(`${API_BASE_URL}/caseroom/${currentCaseroomId}/upload`, {
            method: 'POST',
            body: formData
        });
        const result = await response.json();
        if (result.success) {
            showToast('Document Uploaded & Verified', 'success');
            refreshCaseroom();
        } else {
...
async function triggerSmartExtract() {
    if (!currentCaseroomId) return;

    try {
        showAnalysisLoading('Smart Extract: Aggregating Evidence Intelligence...');
        const response = await fetch(`${API_BASE_URL}/caseroom/${currentCaseroomId}/reanalyze`, {
            method: 'POST'
        });
        const result = await response.json();
        if (result.success) {
            showToast('Case Intelligence Updated from Evidence', 'success');
            // Refresh results screen with new data
            if (result.new_analysis_result) {
                window.caseData = result.new_analysis_result;
                renderFullReport(window.caseData);
            }
            refreshCaseroom();
        } else {
        ...
        showToast('Extraction service unavailable', 'error');
    } finally {
        hideAnalysisLoading();
    }
}
...
function viewDocument(filename) {
    if (!currentCaseroomId) return;
    window.open(`${API_BASE_URL}/caseroom/${currentCaseroomId}/download/${filename}`, '_blank');
}
